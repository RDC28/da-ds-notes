...
# '%' means any number of charcters before or after.
# '_' means only one character before or after.

select * from emp where ENAME like "A%";     # name starts with 'A'.
select * from emp where ENAME like "A%SH";   # name starts with 'A' and ends with 'SH'.
select * from emp where ENAME like "%T";     # name ends with 'T'.
select * from emp where ENAME like "_A%";    # name where second character is 'A'.
select * from emp where ENAME like "%A%";    # name which contains 'A'.
select * from emp where ENAME like "K__L%";  # name where first char is 'K' and fourth char is 'L'.
select * from emp where ENAME like "_%H%_";  # name where 'H' is there but not in first or last place.