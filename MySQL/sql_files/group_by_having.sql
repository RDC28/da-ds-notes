...
# group by clause is used to group rows by values of a certain column.
select EDESIGNATION from emp group by EDESIGNATION;
select EDESIGNATION, count(*) from emp group by EDESIGNATION;
select EDESIGNATION, max(ESAL) as MAXSAL from emp group by EDESIGNATION;
select EDESIGNATION, avg(ESAL) as AVGSAL from emp group by EDESIGNATION;
select EDESIGNATION, sum(ESAL) as SUMOFSAL from emp group by EDESIGNATION;

# having clause only works after group by.
select EDESIGNATION, count(*) from emp group by EDESIGNATION having count(*) > 3;
select EDESIGNATION, max(ESAL) as MAXSAL from emp group by EDESIGNATION having MAXSAL > 50000;
